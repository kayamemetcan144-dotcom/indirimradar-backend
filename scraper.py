...
class ProductScraper:
    ...
    def setup_selenium(self):
        ...
        try: return webdriver.Chrome(service=Service(executable_path=chromedriver_bin), options=options)
        except Exception as e: print(f"Driver Error: {e}"); raise e
        ...
        try:
            ...
            if data['current_price'] == 0:
                ...
                for p in script_prices:
                    if float(p) > 15: data['current_price'] = float(p); break
            if data['current_price'] > 0:
